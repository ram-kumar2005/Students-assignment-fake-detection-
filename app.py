...
# Replace the current CSV setup with this
def init_db():
    if not os.path.exists(users_csv):
        pd.DataFrame([["admin", "admin123", "teacher"]], 
                   columns=["username", "password", "role"]).to_csv(users_csv, index=False)
        
    if not os.path.exists(APP_ROOT / 'submissions.csv'):
        pd.DataFrame(columns=["file", "content", "result", "student"]).to_csv(
            APP_ROOT / 'submissions.csv', index=False)
